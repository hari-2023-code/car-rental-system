...
// Hard-coded super administrator username
const string SUPER_ADMIN_USERNAME = "superadmin";
const string SUPER_ADMIN_PASSWORD = "SuperSecure2025!";  // Change this to your preferred strong password
...
public:
    ...
    static bool isValidDate(int dd, int mm, int yy) {
        if (yy < 1900 || yy > 2030) return false;
...
struct Rental : public Entity {
    ...
    string toString() const override {
        ...
        oss << getId() << "," << carID << "," << customerID << "," << startDate.toString() << ","
            << endDate.toString() << "," << (returnDate.dd == 0 ? "00/00/0000" : returnDate.toString()) << "," 
...
public:
    ...
    bool isValidID(const string& id, const string& prefix) const {
        return id.length() >= 4 && id.substr(0, prefix.length()) == prefix;
    }
    ...
    // Secure password input with asterisk masking and backspace support
  // Secure password input - shows * and handles backspace (works everywhere)
void inputPassword(string& password) {
    ...
    while (true) {
        ...
        if (ch == 8 || ch == 127) {  // Backspace (ASCII 8) or Delete (127)
            if (!password.empty()) {
                password.pop_back();
                cout << "\b \b";  // Move cursor back, print space, move back again
                cout.flush();
            }
        } else if (isprint(ch)) {  // Only printable characters
            password += ch;
            cout << '*';
            cout.flush();
        }
        // Ignore non-printable control keys (arrows, etc.)
    }
}
    ...
    void mainMenu() {
        ...
        while (true) {
            ...
            roleChoice = sanitizeInput(roleChoice);
            ...
            if (roleChoice != "1" && roleChoice != "2") {
                cout << "Invalid choice! Please enter 1 for Admin or 2 for Customer.\n";
                pause();
                continue;
            }

            bool isAdminMode = (roleChoice == "1");

            while (true) {
                clearScreen();
                cout << "\n\t\t=== " << (isAdminMode ? "Admin" : "Customer") << " Panel ===\n\n";
                cout << "1) Login\n";
                cout << "2) Register" << (isAdminMode ? " (Admin Only)" : "") << "\n";
    ...
    void login(bool expectedAdmin) {
        ...
        if (it != users.end()) {
            if ((*it)->isAdmin != expectedAdmin) {
                cout << "Access denied! This account does not match the selected role.\n";
                pause();
                return;
            }

            currentUser = username;
            (*it)->lastLogin = DateManager::getCurrentDate();
            userRepo.save();

            TransactionLogger::logUserAction(username, "Logged in");
            cout << "Login successful! Welcome, " << username << "!\n";
            cout << "Last login: " << (*it)->lastLogin.toString() << "\n";
            pause();
            clearScreen();
    ...
    void registerAdmin() {
        ...
        newUser.username = sanitizeInput(newUser.username);

        if (any_of(userRepo.getAll().begin(), userRepo.getAll().end(),
            [&](const shared_ptr<User>& u) { return u->username == newUser.username; })) {
        ...
        newUser.lastLogin = DateManager::getCurrentDate();

        userRepo.add(make_shared<User>(newUser));
        TransactionLogger::logUserAction(currentUser.empty() ? "System" : currentUser, "Registered new admin: " + newUser.username);
    ...
    void registerCustomer() {
        ...
        userRepo.add(make_shared<User>(newUser));
        TransactionLogger::logUserAction(currentUser.empty() ? "System" : currentUser, "Registered new customer user: " + newUser.username);
        cout << "Registration successful! You can now login as a customer.\n";
        pause();
    }
    ...
    void addCar() {
        ...
        cout << "Enter Year: ";
        while (!(cin >> car.year) || car.year < 1900 || car.year > 2030) {
            cout << "Invalid year! Enter a year between 1900 and 2030: ";
            clearInputBuffer();
        }
        clearInputBuffer();
    ...
    void updateCar() {
        ...
        int year;
        if (cin >> year && year >= 1900 && year <= 2030) {
            car->year = year;
            clearInputBuffer();
        } else {
            clearInputBuffer();
    ...
    void rentCar() {
        ...
        if (discount > 0) cout << " (Loyalty Discount Applied: " << (discount * 100) << "%)";
    ...
    void viewMyRentals() {
        string customerID;
        cout << "Enter your Customer ID (start with CU): ";
        ...
        for (const auto& rental : rentals) {
            if (rental->customerID == customerID && rental->status == "Active") {
                rental->display();
                found = true;
            }
        }
    ...
    void extendRental() {
        ...
        if (totalDays > settings.getMaxRentalDays()) {
            cout << "Extended period exceeds maximum allowed days (" << settings.getMaxRentalDays() << ")!\n";
            pause();
            return;
        }
    ...
    void returnCar() {
        ...
        if (lateDays > 0) {
            ...
            TransactionLogger::logUserAction(currentUser, "Applied late penalty of $" + to_string(rental->latePenalty) + " for rental " + rentalID);
        }
        ...
        if (rental->latePenalty > 0) cout << "Late Penalty: $" << fixed << setprecision(2) << rental->latePenalty;
        cout << endl;
        pause();
    }
    ...
    void updateCustomerProfile(bool isAdmin = false) {
        ...
        input = sanitizeInput(input);
        if (!input.empty()) {
            if (isValidEmail(input)) customer->email = input;
            else cout << "Invalid email! Keeping original.\n";
        }
        ...
        input = sanitizeInput(input);
        if (!input.empty()) {
            if (isValidPhone(input)) customer->phone = input;
            else cout << "Invalid phone! Keeping original.\n";
        }
        ...
        customerRepo.save();
        TransactionLogger::logUserAction(currentUser, "Updated customer profile " + customerID);
    ...
    void updateMaintenance() {
        ...
        cout << "Enter new Date (dd mm yyyy) (" << record->maintenanceDate.toString() << "): ";
        int dd, mm, yy;
        ...
        cout << "Enter new Status (Scheduled/Completed) (" << record->status << "): ";
        getline(cin, input);
        input = sanitizeInput(input);
    ...
    void viewCarMaintenanceHistory() {
        string carID;
        cout << "Enter Car ID to view history: ";
        getline(cin, carID);
        carID = sanitizeInput(carID);
        if (!isValidID(carID, "CR")) {
            cout << "Invalid Car ID!\n";
        ...
        if (!found) cout << "No maintenance records found.\n";
        pause();
    }
    ...
    void viewRentalHistory() {
        ...
        clearScreen();
        cout << "=== Rental History for Customer " << customerID << " ===\n\n";
        auto rentals = rentalRepo.getAll();
        bool found = false;
        ...
        if (!found) cout << "No rental history found.\n";
        pause();
    }
    ...
    void viewLoyaltyPoints() {
        ...
        clearScreen();
        cout << "=== Loyalty Points ===\n\n";
        cout << "Customer: " << customer->fullName << " (" << customer->getId() << ")\n";
        cout << "Loyalty Points: " << customer->loyaltyPoints << endl;
        cout << "Eligible for Discount: " << (customer->loyaltyPoints >= 1000 ? "Yes (" + to_string(int(settings.getLoyaltyDiscountRate() * 100)) + "%)" : "No") << endl;
        pause();
    }
    ...
    void generateCustomerLoyaltyReport() {
        ...
        TransactionLogger::logUserAction(currentUser, "Generated loyalty report");
        pause();
    }

    void monthlyReport() {
        clearScreen();
        cout << "=== Monthly System Report ===\n\n";
        float totalRevenue = 0.0f, totalPenalties = 0.0f, totalMaintenanceCost = 0.0f;
        int active = 0, completed = 0, cancelled = 0, scheduledMaint = 0, completedMaint = 0;
        ...
        for (const auto& m : records) {
            ...
            if (m->status == "Scheduled") scheduledMaint++;
            else if (m->status == "Completed") completedMaint++;
        }

        cout << "Rentals Summary:\n";
        cout << "  Active: " << active << " | Completed: " << completed << " | Cancelled: " << cancelled << endl;
        cout << "Financial Summary:\n";
        cout << fixed << setprecision(2);
        cout << "  Total Revenue: $" << totalRevenue << endl;
        cout << "  Total Penalties: $" << totalPenalties << endl;
        cout << "  Total Maintenance Cost: $" << totalMaintenanceCost << endl;
        cout << "  Net Income: $" << (totalRevenue + totalPenalties - totalMaintenanceCost) << endl;
        cout << "Maintenance Summary:\n";
        cout << "  Scheduled: " << scheduledMaint << " | Completed: " << completedMaint << endl;
